...
\newcommand{\Liam}[1]{{\color{darkgray}Liam: #1}}
...
\begin{enumerate}
      ...
      \item Line 92-94, same section. You give two statements:
            ...
            \Liam{that's a nice observation. could be mentioned as an `intuitively, this should correspond to merging of homologous stretches of sequence in roughly reverse evolutionary order'. Risk is suggesting that this is the reason for doing it that way?}
...
\Marco{I agree, I think it's a different design choice. We could also mention that the marginalize command has an option to project out paralogs, but should be a post-processing step rather than something that we want to do while building the graph.}
...
\Marco{if the fracturing derives from a real structural property of pangenomes (i.e. ISs that really move around and split long blocks) this might be an intrinsic real obstacle.}\\
