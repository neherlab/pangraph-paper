...
\responsedraft{
      ...
      Currently two different ways exist of performing some effective form of paralog splitting. The first one consist in tuning

      In the future we plan on providing an option to perform a more robust form of paralag splitting. This could be done using both topological information on the neighborhood of a duplicated pancontig, as currently done in the \texttt{marginalize} command, and phylogenetic information, by building a phylogenetic tree from the alignment of each pancontig and splitting diverged branches. This functionality is not however present in the current version of \textit{PanGraph}.
      ...
      Some form of PanX postprocessing paralog split. Build trees for every pancontig and define some split criterion.
      Partially done in marginalize (\texttt{--reduce-paralog} flag)
}
