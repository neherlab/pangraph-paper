...
Using \textit{PanGraph}, we build multiple pangenome graphs for each species in the dataset. We test all the different alignment kernels (\textit{minimap2} with \textit{asm10} or \textit{asm20} option, or \textit{mmseqs2}). Moreover we also vary the value of the pseudo-energy parameters $\alpha,\beta$ from eq.~(\ref{eq:pseudo-energy}), using either the standard values ($\alpha=100$, $\beta=10$) or null values ($\alpha = \beta = 0$).\\
\textit{PanGraph} can build pangenome graphs comprising hundreds of isolates in a few hours (5-6h for 307 EC isolates with \textit{minimap2} kernel, cf. Fig~\ref{fig:panx-benchmark}A). Use of \textit{mmseqs2} consistently requires longer time, but provides higher accuracy when merging highly diverged sequences. This is the case for the HP dataset, whose core genome divergence sits at the limit of what the \textit{minimap2} kernel can accurately align. In this case the use of \textit{mmseqs2} improves both sequence compression and core pangenome fraction (cf. Fig~\ref{fig:panx-benchmark}B-C).\\
% beta poses a limit to the divergence. Removing it..
% but increases fragmentation (cite supplementary)
% talk about PM
% average compression
% core genome fraction compatible?
