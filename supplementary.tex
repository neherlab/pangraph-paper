...
% TODO: also report average core genome size?
% TODO: evaluate core genome divergence? Or average mash triangle distance?
% TODO: avoid talking about PanX?
% TODO: add link with accesion numbers and download?

\begin{table}[hb]
    \begin{tabular}{l l l l}
        \hline\hline
        Species         & N. isolates & Avg. chromosome length [Mbp] & Avg. pairwise divergence (core genes) \\
        \hline
        E.~Coli         & 307         & 5.0                          &                                       \\
        K.~Pneumoniae   & 109         & 5.3                          &                                       \\
        H.~Pylori       & 85          & 1.6                          &                                       \\
        M.~Tuberculosis & 51          & 4.4                          &                                       \\
        P.~Marinus      & 10          & 1.8                          &                                       \\
        \hline
    \end{tabular}
    \caption{{\bf Dataset overview}. We downloaded complete chromosome sequences from RefSeq for 5 different bacterial species. For each species we report the number of isolates downloaded, along with the average chromosome length and average pairwise divergence on core genes.}
...
In Fig.~\ref{fig:panx-benchmark-suppl} we display the results of the benchmakr. On every panel we display on the x-axis results for different species, and indicate with different colors the five possible choices for the alignment kernel.

\begin{figure}[htb]
    \includegraphics[width=\textwidth]{figs_suppl/panx_benchmark_suppl.pdf}
    \caption{{\bf Benchmark on real data}. We run \textit{PanGraph} on datasets consisting of 5 different bacterial species, and using 5 different options for the alignment kernel as described in the text.
...
% TODO: explain the sense of L50 and N50


\section{Graph marginalization}

% TODO: simplify dataset descritpion (is already described above)
