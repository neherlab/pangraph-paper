...
These data were previously analyzed using PanX \cite{ding2018panx}, a pipeline for the analysis of bacterial pangenome. The pipeline takes as input a set of annotated genomes, and it clusters and aligns homologous genes. This allowed us to identify core genes (i.e. genes present in all isolates), and evaluate their average pairwise sequence divergence $\avg{d_\text{core}}$. We also evaluated the average genome length $\avg{\Lgen}$, the total pangenome length $\Lpang$, the total core genome length $\Lcore$, and the total soft-core genome length $\Lsoftcore$, where soft-core genes are defined as genes being present in at least 95\% of the isolates. These quantities are reported in Table~I in the main text.\\
...
We point out that the core genome size of the EC dataset found by PanX is significantly smaller than typical values seen in the literature, which are closer to 2 Mbp (see for example \cite{horsfield2023accurate}).\\
We point out that the value of $\Lcore = 0.7$ Mbp for EC is likely to be biased by annotation errors, to which the PanX pipeline is sensitive. Values for the core genome size of E.~coli reported in the literature are usually between 1500 and 2500 genes \cite{sutton2021pan,horsfield2023accurate,park2019large}, depending on the core genome definition used and the method used to mitigate annotation errors. For example using a strict definition for core genes the authors in \cite{sutton2021pan} initially find a core genome size of $\sim 700$ genes on a dataset of $\sim 1000$ isolates. However correcting for annotation errors this estimate increases to $\sim 1500$ genes. In our case we do not correct for these errors, since we only use core gene alignment to estimate the diversity of our dataset and since PanGraph is not sensitive to annotation errors. But we point out that for the EC dataset the value of $\Lsoftcore$ is likely to provide a better estimate of the core genome size.
