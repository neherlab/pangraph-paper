...
\begin{enumerate}
    ...
    \item For each of the remaining matches we attempt to merge pancontigs following these steps:
          \begin{enumerate}
              ...
              \item The CIGAR string of the match is then parsed and subdivided in sub-regions. Regions can be either shared or private to query or reference. Shared regions contain Each insertion or deletion longer than $\Lthr$ will cause a split in the merge, and will be stored as a separate panconting. \textbf{(what happens with small matches in longer flanking indels?)}
              \item
          \end{enumerate}
    \item Once the list of all matches has been exhausted and all merges have been performed, the two graphs have been turned into
\end{enumerate}

% post-processing step: transitive edges. Explain.

% blocks become progressively deeper but shorter. The energy parameters and the threshold lengt determine when to stop.
