...
\begin{enumerate}
    ...
    \item For each of the remaining matches we attempt to merge pancontigs following these steps:
          \begin{enumerate}
              \item We check that no prior merges have been performed on the same two pancontigs. Sometimes multiple matches are found on the same pancontig, and in this case we prioritize the ones with higher pseudo-energy. Any following match on the same pancontig will be discarded at this step. It will however be performed later on in the self-map phase of the algorithm (see step 4).
